...
class TestListQuery(unittest.TestCase):
    ...
    def testListQuery(self):
        for planet, probe in PERSISTED_DATA.items():
            ...
            self.bw_client.publish(uri, payload_objects=(po,), persist=True, auto_chain=True)

        results = self.bw_client.query(BASE_URI + "/persisted/+", auto_chain=True)
        ...
        children = self.bw_client.list(BASE_URI + "/persisted", auto_chain=True)
